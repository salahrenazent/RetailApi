...
namespace RetailApi.DAL.Services
{
    public class EmployeeDepartmentService:IEmployeeDepartmentService
    {
        ...
        {
            List<EmployeeDepartment> departmentList = new List<EmployeeDepartment>();
            using (SqlConnection connection = ADO.GetConnection())
            {
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = connection;
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "SP_EMPLOYEE_DEPARTMENT";
                ...
                {
                    departmentList.Add(new EmployeeDepartment
                    {
        ...
        public Int32 SaveData(EmployeeDepartment department)
        {
            try
            {
        ...
        public EmployeeDepartment GetItems(int id)
        {
            ...
            try
            {
                string strSQL = "SELECT TB_EMPLOYEE_DEPARTMENT.ID,TB_EMPLOYEE_DEPARTMENT.CODE,TB_EMPLOYEE_DEPARTMENT.DEPT_NAME, " +
               "TB_EMPLOYEE_DEPARTMENT.COMPANY_ID," +
               "TB_COMPANY.COMPANY_NAME " +
               "FROM TB_EMPLOYEE_DEPARTMENT " +
               "INNER JOIN TB_COMPANY ON TB_EMPLOYEE_DEPARTMENT.COMPANY_ID = TB_COMPANY.ID " +
               "WHERE TB_EMPLOYEE_DEPARTMENT.ID =" + id;

                DataTable tbl = ADO.GetDataTable(strSQL, "EmployeeDepartment");
                if (tbl.Rows.Count > 0)
                {
